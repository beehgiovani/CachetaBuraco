-- O cliente (GameRulesEngine.kt) ja tinha esse mesmo furo: checkSequencia so
-- exigia minimo 3 cartas, sem maximo, entao dava pra baixar sequencia de 4+
-- ou encaixar carta num jogo ja fechado (o encaixe revalida meld+carta como
-- jogo novo). O trinca ja travava em 3 (cbr_check_trinca com exact_size),
-- mas a sequencia nao. Bruno reportou jogando Cacheta online (2026-08-11).
-- Corpo copiado verbatim de 0015_structural_validation_lint_fixes.sql, so
-- acrescentando a checagem de tamanho total logo na entrada do bloco CACHETA.
...
begin
    ...
    if p_game_type = 'CACHETA' then
        ...
        -- jogos de 3 e jogo baixado nao cresce. Uma unica checagem aqui cobre
        -- as duas regras: baixar 4+ e recusado, e "encaixar" numa trinca ou
        -- sequencia ja baixada tambem, porque o encaixe manda meld+carta nova
        -- como jogo inteiro e cairia em 4 cartas.
        if coalesce(array_length(v_cards, 1), 0) <> 3 then
            return jsonb_build_object('valid', false, 'reason', 'Na Cacheta, cada jogo tem exatamente 3 cartas');
        end if;
