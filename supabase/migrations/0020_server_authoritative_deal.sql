-- Fase 1 da autoridade do baralho no servidor.
--
-- Ate agora eu deixava o host embaralhar e distribuir as cartas no proprio
-- aparelho (Kotlin) e so mandava o resultado pro banco via GAME_START; o
-- servidor so validava jogada DEPOIS que a mesa ja existia (migrations
-- 0014/0016). Um host malicioso ainda conseguia manipular o embaralhamento ou
-- ver a mao alheia antes da hora.
--
-- Criei `public.start_online_round` pra embaralhar e distribuir as cartas
-- DENTRO do Postgres nas 3 modalidades (Cacheta, Buraco, Tranca), publicando os
-- mesmos eventos GAME_START/PUBLIC_STATE que eu ja mandava do host --
-- reaproveitei o `append_match_event` (0013) pra nao duplicar a validacao de
-- identidade/round que ja tinha. O guard de round (0017) e o ledger privado
-- (0016) continuam funcionando sem eu mudar nada neles, porque disparam no
-- mesmo INSERT em match_events, nao importa quem chamou.
--
-- Escopo desta fase: so a distribuicao inicial da rodada (baralho, mao, vira,
-- mortos, lixo de abertura). Deixei compra/descarte/baixa durante a rodada como
-- estavam, passando pela validacao que ja existe (0014/0016) e que ja e
-- server-side.
--
-- IMPORTANTE: isto so cria a funcao no banco, eu ainda nao liguei o app nela.
-- O OnlineNetworkRepository/MatchViewModel continuam montando a mao localmente
-- por enquanto -- vou trocar isso na fase 2, separada, depois de revisar esse
-- desenho com calma e rodar um smoke test manual. Wi-Fi local e partida contra
-- a maquina eu nao toquei: continuam com o host distribuindo localmente,
-- porque ali nao tem servidor nenhum pra ser autoridade.

create or replace function private.cbr_shuffled_deck()
returns text[]
language sql
stable
set search_path = ''
as $$
    select array_agg(card_id order by random())
    from (
        select rank_name || '_' || suit_name || '_' || color_name as card_id
        from unnest(array[
            'ACE','TWO','THREE','FOUR','FIVE','SIX','SEVEN',
            'EIGHT','NINE','TEN','JACK','QUEEN','KING'
        ]) as rank_name
        cross join unnest(array['HEARTS','DIAMONDS','CLUBS','SPADES']) as suit_name
        cross join unnest(array['RED','BLACK']) as color_name
    ) as cards;
$$;

comment on function private.cbr_shuffled_deck() is
    'Duas copias completas (RED/BLACK) de 52 cartas, sem coringa impresso -- '
    'mesma composicao de DeckFactory.createDecks(2, includeJokers=false).';

create or replace function private.cbr_is_red_three(p_card_id text)
returns boolean
language sql
immutable
set search_path = ''
as $$
    select p_card_id like 'THREE_HEARTS_%' or p_card_id like 'THREE_DIAMONDS_%';
$$;
...
begin
    ...
    -- local. Fica marcado no roadmap que mover o SERVE_MORTO para o servidor
    -- tambem e trabalho futuro (fase 3).
...
comment on function public.start_online_round(uuid) is
    'Fase 1 da autoridade do servidor: embaralha e distribui a rodada dentro do '
    'Postgres. Ainda nao chamada pelo app -- fase 2 troca o cliente para pedir '
    'a distribuicao aqui em vez de montar a mao localmente no transporte online.';

revoke all on function private.cbr_shuffled_deck() from public;
revoke all on function private.cbr_is_red_three(text) from public;
