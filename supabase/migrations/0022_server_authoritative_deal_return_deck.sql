-- Faltou uma coisa antes de eu ligar o app nisso: eu so devolvia `deckSize`
-- (a contagem), nao as cartas que sobraram no monte. Mas o host continua
-- puxando carta localmente durante a partida (compra do monte, reciclagem do
-- lixo na Cacheta) -- isso e fase 2+ pra virar servidor tambem, fora do escopo
-- de hoje. Sem a ordem real do monte restante, o host ficava sem baralho pra
-- puxar depois do primeiro sorteio.
--
-- Adiciono `deck` (as cartas que sobraram, na mesma ordem que o topo continua
-- sendo o ultimo elemento) na resposta que so o host recebe. Isso nao piora a
-- seguranca de nada que ja existia: a embaralhada e a mao inicial continuam
-- 100% decididas aqui dentro antes do host ver qualquer coisa -- so a compra
-- durante a rodada continua no mesmo nivel de confianca no host que sempre
-- teve (validado depois pelas migrations 0014/0016, como qualquer outra
-- jogada).
